<template>
	<view class="wrapper">
		...
		<view class="selectContent" v-show="videoItem.length" v-if="isshowSelect">
			<span v-if="showSelected">恭喜!抽中了<span
					style="color: #CE5353;margin:0 18rpx;font-size:22px;font-weight: bold;">{{selected}}</span>!</span>
			<span v-if="showIng">正在抽奖...</span>
			<span v-if="showStart">点开始按钮抽奖</span>
		</view>
		<view class="ul" v-show="videoItem.length" v-if="isshowSelect">
			<li :class="index<3?`li${index+1}`:
									index==3?`li8`:
									index==4?`start`:
									index==5?`li4`:
									index>5?`li${13-index}`:
									`li${index}`" v-for="item,index in selectContent" :key="item" @click="handleClick(index)">
				{{selectContent[index]}}</li>
		</view>
		<uni-popup ref="inputDialog" type="dialog">
			<uni-popup-dialog mode="input" placeholder="填写你想选择的事件名!" message="成功消息" :duration="2000"
